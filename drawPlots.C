...
void drawCMSLatex(double luminosity, double height){
  ...
  lumitex = new TLatex(.9-gPad->GetLeftMargin(), 1-gPad->GetTopMargin(), Form("%.1f pb^{-1} (13 TeV)", luminosity*1000) );    
...
void drawPlots(TString config_file)
{
  ...
  TGaxis::SetExponentOffset(-0.07, 1, "y"); // X and Y offset for Y axis
  TGaxis::SetExponentOffset(-1, 0.05, "x"); // X and Y offset for X axis
